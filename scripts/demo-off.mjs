...
if (fs.existsSync(BACKUP_PATH)) {
  ...
  for (const cat of backup.categoryNames ?? []) {
    db.prepare('UPDATE category_config SET category_name = ?, updated_at = ? WHERE category_id = ?')
      .run(cat.category_name, now, cat.category_id);
  }
  fs.unlinkSync(BACKUP_PATH);
  const names = backup.members.map((m) => `${m.member_key} → ${m.name}`).join(', ');
  console.log(`Real names restored: ${names}`);
  if ((backup.categoryNames ?? []).length > 0) {
    console.log(`Category names restored: ${backup.categoryNames.map((c) => c.category_name).join(', ')}`);
  }
} else {
