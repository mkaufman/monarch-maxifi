...
### Fixed budget validation warning

The sum of all Fixed Subcategory fields plus the sum of Special Expenses for the selected year must equal the Fixed Total entered in Settings. If they diverge, the budget comparison will be silently wrong.

**Rule:** when `subcategoryTotal + specialExpensesTotal ≠ fixedTotal`, show a warning at the bottom of the Fixed Subcategories card in Settings.

**Conditions to show the warning:** both `fixedTotal > 0` and `subcategoryTotal + specialExpensesTotal > 0` (suppress when the user hasn't entered anything yet).

**Wording:**
- If line items exceed Fixed Total: *"Line items total {$X} — {$Y} over your Fixed Total of {$Z}."*
- If line items fall short: *"Line items total {$X} — {$Y} under your Fixed Total of {$Z}."*

**Implementation:** Pure frontend change in `components/SettingsForm.tsx`. No API or DB changes needed. Derive `subcategoryTotal` from the `subcategories` state, `specialExpensesTotal` from `specialExpenses` filtered to `year === budgetYear`, and compare against `getBudgetAmount('fixed')`.
...
- ⚠️ Fixed subcategories + Special Expenses don't match Fixed Total (see Fixed budget validation warning under Special Expenses)
