...
export async function GET(request: NextRequest) {
  ...
  // Settings edits the real DB, so it passes ?raw=1 to bypass the demo fixture.
  // Without it, the form would load demo values and a save would clobber real data.
...
export async function PUT(request: NextRequest) {
  const body = await request.json() as {
    ...
    subcategories?: Array<{ subcategory: string; amount: number; actual_amount?: number | null }>;
  ...
  if (body.subcategories) {
    for (const s of body.subcategories) {
      upsertMaxiFiSubcategory({ year: body.year, subcategory: s.subcategory, amount: s.amount, actual_amount: s.actual_amount ?? null, updated_at: now });
    }
  }
