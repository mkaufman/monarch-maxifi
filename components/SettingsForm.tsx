...
function getSubcatOptions(person1: string, person2: string) {
  return [
    ...
    { value: 'other_expenses', label: 'Other / Special Expenses' },
  ];
}
...
// Subcategories Monarch's expense feed can't see (payroll deductions / transfers),
// so their actuals are entered manually in the Contributions section.
const CONTRIBUTION_SUBCATS = ['person1_retirement', 'person2_retirement', 'hsa_contributions'];
...
export default function SettingsForm() {
  ...
  const load = useCallback(async (yr: number) => {
    ...
    try {
      const [cfgRes, budRes, membRes] = await Promise.all([
        fetch('/api/config'),
        fetch(`/api/budgets?year=${yr}&raw=1`), // raw=1 → real DB values, not demo fixture (Settings edits real data)
        fetch('/api/members'),
      ]);
    ...
    try {
      await fetch('/api/budgets', {
        ...
        body: JSON.stringify({
          ...
          subcategories: subcategories.map((s) => ({ subcategory: s.subcategory, amount: s.amount, actual_amount: s.actual_amount ?? null })),
  ...
  const setSubcatAmount = (sub: string, amount: number) => {
    setSubcategories((prev) => {
      ...
      return [...prev, { year: budgetYear, subcategory: sub, amount, actual_amount: null, updated_at: new Date().toISOString() }];
    });
  };

  const getSubcatActual = (sub: string) =>
    subcategories.find((s) => s.subcategory === sub)?.actual_amount ?? 0;

  const setSubcatActual = (sub: string, actual: number) => {
    setSubcategories((prev) => {
      const existing = prev.find((s) => s.subcategory === sub);
      const actual_amount = actual === 0 ? null : actual;
      if (existing) return prev.map((s) => (s.subcategory === sub ? { ...s, actual_amount } : s));
      return [...prev, { year: budgetYear, subcategory: sub, amount: 0, actual_amount, updated_at: new Date().toISOString() }];
    });
  };
  ...
  const knownSubcatKeys = new Set(subcatOptions.map((o) => o.value));
  const subcategoryTotal = subcategories
    .filter((s) => knownSubcatKeys.has(s.subcategory))
    .reduce((sum, s) => sum + s.amount, 0);
  const specialExpensesYearTotal = specialExpenses
    .filter((se) => se.year === budgetYear)
    .reduce((sum, se) => sum + se.amount, 0);
  const lineItemTotal = subcategoryTotal + specialExpensesYearTotal;
  const fixedTotal = getBudgetAmount('fixed');
  const fixedBudgetDelta = lineItemTotal - fixedTotal;
  const showFixedBudgetWarning = fixedTotal > 0 && lineItemTotal > 0 && fixedBudgetDelta !== 0;
    ...
    <div className="space-y-10">
      ...
      <section className="space-y-6">
        ...
        <div className="bg-surface rounded-xl border border-border p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            {Object.entries(subcatLabels)
              .filter(([key]) => !CONTRIBUTION_SUBCATS.includes(key))
              .map(([key, label]) => (
                <DollarInput
                  key={key}
                  label={label}
          ...
          {showFixedBudgetWarning && (
            <p className="text-sm text-warning bg-warning/10 border border-warning/30 rounded-lg px-3 py-2">
              Line items total {fmt(lineItemTotal)} —{' '}
              {fixedBudgetDelta > 0
                ? <>{fmt(fixedBudgetDelta)} over your Fixed Total of {fmt(fixedTotal)}.</>
                : <>{fmt(Math.abs(fixedBudgetDelta))} under your Fixed Total of {fmt(fixedTotal)}.</>
              }
            </p>
          )}
        </div>

        {/* Contributions (manually tracked — Monarch can't see payroll deductions / transfers) */}
        <div className="bg-surface rounded-xl border border-border p-6 space-y-4">
          <div>
            <h3 className="font-semibold text-navy flex items-center">
              Contributions
              <Tooltip content="Retirement and HSA contributions come from payroll deductions or account transfers, which Monarch doesn't report as spending. Enter your annual contribution so the report can compare it to your MaxiFi plan." />
            </h3>
            <p className="text-xs text-text-secondary mt-0.5">Monarch can&rsquo;t see these — enter what you contribute so it counts toward your Fixed plan.</p>
          </div>
          <div className="space-y-2">
            <div className="flex items-center gap-3 text-xs text-text-secondary">
              <span className="flex-1"></span>
              <span className="w-32 text-right">Plan</span>
              <span className="w-32 text-right">Contributed</span>
            </div>
            {CONTRIBUTION_SUBCATS.map((key) => (
              <div key={key} className="flex items-center gap-3">
                <span className="flex-1 text-sm text-text-secondary">{subcatLabels[key]}</span>
                <div className="w-32"><MoneyField value={getSubcatAmount(key)} onChange={(v) => setSubcatAmount(key, v)} /></div>
                <div className="w-32"><MoneyField value={getSubcatActual(key)} onChange={(v) => setSubcatActual(key, v)} /></div>
              </div>
            ))}
          </div>
        </div>
...
// Bare dollar input (no stacked label) — used in table-style rows like Contributions.
