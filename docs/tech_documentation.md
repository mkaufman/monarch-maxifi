...
The `/api/monarch` and `/api/budgets` routes check for the presence of fixture files (`data/demo-monarch.json`, `data/demo-budgets.json`) at the top of each request. If found, the fixture is returned immediately instead of fetching live data. This enables screenshot-safe demo mode without a server restart.

Demo mode is activated and deactivated via:
```bash
npm run demo:on   # scales all amounts, replaces names, writes fixtures
npm run demo:off  # removes fixtures, restores real names
```
