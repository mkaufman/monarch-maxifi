...
> **Note (July 2026):** Monarch Money has temporarily suspended their MCP (Model Context Protocol) integration while working through questions raised by one of their data providers. The live data feed this app depends on is currently unavailable. Monarch has said they expect to restore it soon. This notice will be removed once MCP access is restored.
...
**The app shows an error and live data won't load (as of July 2026).**
Monarch Money has temporarily suspended their MCP integration. This is not a problem with your setup — the underlying API is unavailable for all users. Watch for an update from Monarch on when access will be restored.
