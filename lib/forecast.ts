...
export function computeForecast(input: ForecastInput): ForecastResult {
  ...
  switch (model) {
    ...
    case 'adjusted_run_rate': {
      ...
      // Exclude one-time amounts from the run-rate base so they aren't annualized,
      // then add the known one-time / future amount once to the projected total.
      const runRateBase = ytdTotal - (excludeAmount ?? 0);
      return { annualForecast: (runRateBase / daysElapsed) * 365 + (addAmount ?? 0), model };
    }
